...
Mouse right-button input is arbitrated entirely in the renderer as a local
click-or-pan gesture. Until movement reaches 4 CSS pixels it leaves the camera
unchanged and preserves the normal board context click. Crossing that threshold
activates pan from the original pointer-down coordinate and suppresses only the
context-menu event generated by that drag. This gesture is available in
read-only mode, never enters the selected tool, and changes only local camera
and transient input state; it does not enter the CRDT, awareness, undo, or wire
protocol.
...
its coordinate buffer and flattened Konva buffer grow incrementally instead of
being rebuilt for every pointer event. A one-million-point emergency threshold
progressively compacts an individual local laser path; it is a resource guard
for an abnormally long uninterrupted gesture, not a normal drawing limit.

A separately derived awareness projection carries rolling tails from at most
the newest 16 strokes and 160 aggregate points. Every session and stroke has a
stable stream ID, and every tail carries the absolute offset of its first point.
Successive windows overlap, so a receiver merges only the new suffix, repairs a
moving endpoint in place, retains paths which no longer fit in the newest-stroke
packet, and never connects separate strokes. Missing intermediate packets are
normally recovered by the overlap in a later window; a genuine gap starts from
the next received window without inventing a connecting segment. One packet is
therefore bounded while the complete remote session is accumulated. The web
renderer reuses its Konva nodes, indexes retained strokes by stream ID, and
updates only changed geometry. As hostile-input guards, one remote session may
retain at most 1,024 strokes and 131,072 preview points before oldest-stroke
eviction or progressive geometry compaction. Each retained stroke keeps its own
...
the rolling awareness preview carries the same visible translation. The last
...
- Drawing modifier ordering: pre-held standalone `Alt` latches an
  ...
  color/width/opacity snapshots, rolling 16-stroke/160-point awareness tails,
  offset-based remote accumulation and node reuse, stationary remote retention, and
